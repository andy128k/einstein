...
impl Widget<String> for InputField {
    ...
    fn draw(&self, resource_manager: &dyn ResourceManager) -> Brick {
        ...
        let mut brick = Brick::new(self.get_size().width, self.get_size().height)
            .border(Border::Beveled(
                Color::RGB(128, 128, 128),
                Color::RGB(255, 255, 255),
            ))
            .text(
                Text::new(&self.text.borrow())
                    .font_size(font_size)
        ...
        if self.cursor_visible.get() {
            ...
            let cursor = Brick::new(2, self.get_size().height - 8)
                .background(Background::Color(Color::RGB(33, 33, 33)));
...
mod test {
    ...
    use crate::error::*;
    use crate::ui::context::{Context, MainLoopQuit};
    ...
    impl Context for ContextMock {
        ...
        fn main_loop(&self, _widget: &mut dyn Widget<MainLoopQuit>) -> Result<()> {
            unreachable!()
        }
    }
    ...
    fn test_input() {
        ...
        field
            .on_event(&Event::TextInput("A".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "A");

        field
            .on_event(&Event::KeyDown(Keycode::Backspace), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "");

        field
            .on_event(&Event::TextInput("B".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "B");

        field
            .on_event(&Event::TextInput("o".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "Bo");

        field
            .on_event(&Event::TextInput("ы".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "Boы");

        field
            .on_event(&Event::TextInput("y".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "Boыy");

        field
            .on_event(&Event::KeyDown(Keycode::Left), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "Boыy");
        ...
        field
            .on_event(&Event::KeyDown(Keycode::Right), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "y");
        ...
        field
            .on_event(&Event::TextInput("e".to_owned()), &context)
            .unwrap();
        assert_eq!(&*field.text.borrow(), "ye");

        field
            .on_event(&Event::TextInput("s".to_owned()), &context)
            .unwrap();
        ...
        field
            .on_event(&Event::KeyDown(Keycode::Delete), &context)
            .unwrap();
        ...
        field
            .on_event(&Event::TextInput("4".to_owned()), &context)
            .unwrap();
