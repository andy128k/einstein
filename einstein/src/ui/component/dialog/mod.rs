...
pub fn dialog_widget<B: Into<Option<Background>>, A, W: Widget<A> + 'static>(
    background: B,
    widget: W,
) -> Container<A> {
    ...
    Container::<A>::modal(screen, background).add(x, y, widget)
}
    ...
    A: Clone,
{
    ...
    ConditionalWidget::new(condition.clone(), factory).flat_map_action(move |action, _| {
